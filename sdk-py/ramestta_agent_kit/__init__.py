...
NETWORKS: Dict[str, Dict[str, Any]] = {
    ...
    "mainnet": {
        ...
        "boot_helper": "0x0781EAc0486cB177864586e4DfC2077E8B88bBEa",
        ...
        "memory": "0x571e0C76594348038ed4B9361211Ea2A50bd24ac",
        "reputation": "0x774a0da308cD92a09BCF08ff896733fdBDC7786a",
...
class Agent:
    ...
    def boot(cls, name: str, private_key: str, network: str = "testnet",
        ...
        # M-07/H-05: no predictable default. Domain-separated deterministic mesh key
        # (EIP-712, bound to chain + name) — matches mesh_keys()/send_message() so the
        # published key and the one used to decrypt are always the same.
        key = x25519_key or _mesh.derive_mesh_keys(
            private_key, net["chain_id"], name, net["registry"]).public_raw
    ...
    def mesh_keys(self):
        """This agent's DOMAIN-SEPARATED X25519 mesh keypair (EIP-712, bound to
        chain + agent name — not reproducible via a generic message signature)."""
        from . import mesh as _mesh
        return _mesh.derive_mesh_keys(
            self._account.key.hex(), self._net["chain_id"], self.name, self._net["registry"])
    ...
    async def send_message(self, to_name: str, message: str) -> str:
        ...
        client = _mesh.MeshClient(self._account.key.hex(), self.name,
                                  self._net["chain_id"], self._net["registry"])
